...
public class Puffy_Cloud : MonoBehaviour {
	...
	void Build() {
		...
		if(positionSeed){
			randomSeed = (int)(_transform.position.x*10) + ((int)_transform.position.y*10) + ((int)_transform.position.z*10);
		}
		...
		for(i=0;i < baseCount;i++){
			...
			if(index >= 0){
				...
				for(j=0;j<subCount;j++){
					...
					Random.seed = randomSeed + 1357 + j*i;
